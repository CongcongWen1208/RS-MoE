...
class Blip2VicunaInstructLoRA(Blip2Base):
    ...
    def __init__(
        ...
        print('self.ln_vision: ', self.ln_vision)
        if freeze_vit:
        ...
        #print('LlamaTokenizer.from_pretrained: ',LlamaTokenizer.from_pretrained)
        self.llm_tokenizer = LlamaTokenizer.from_pretrained(llm_model, use_fast=False, truncation_side="left")
        # print('self.llm_tokenizer: ',self.llm_tokenizer)
        # print('type(self.llm_tokenizer): ',type(self.llm_tokenizer))
        #print(' LlamaForCausalLM.from_pretrained: ', LlamaForCausalLM.from_pretrained)
        '''LlamaTokenizer.from_pretrained:  <bound method PreTrainedTokenizerBase.from_pretrained of <class 'transformers.models.llama.tokenization_llama.LlamaTokenizer'>>
            LlamaForCausalLM.from_pretrained:  <bound method PreTrainedModel.from_pretrained of <class 'lavis.models.blip2_models.modeling_llama.LlamaForCausalLM'>>'''
        self.llm_model = LlamaForCausalLM.from_pretrained(
            llm_model, torch_dtype=torch.float16 #,cache_dir='/scratch/sg7484/ra/data/LAVIS/output_vicuna'
        )

        print('self.llm_model: ',self.llm_model)
        ...
        print('self.Qformer.config: ',self.Qformer.config)
        print('self.llm_model.config.hidden_size: ',self.llm_model.config.hidden_size)
        print('self.llm_model: ',self.llm_model)
        self.llm_proj = nn.Linear(
            self.Qformer.config.hidden_size, self.llm_model.config.hidden_size
        )
        #print('self.llm_proj: ',self.llm_proj)
    ...
    def forward(self, samples):
        ...
        with torch.no_grad():
            for index, out in enumerate(logits):
                scores_out = F.softmax(out, dim=1)
                #print('scores_out: ',scores_out)
                labels_out = torch.argmax(scores_out,dim=1)
                #print('labels_out: ',labels_out)
                labels_out[labels_out == 0] = 2 # convert output id 0 to 2 (eos_token_id)
                output_text = self.llm_tokenizer.batch_decode(labels_out, skip_special_tokens=True)
        ...
        print('samples: ',samples['text_output'][index])
    ...
    def from_config(cls, cfg):
        ...
        max_txt_len = cfg.get("max_txt_len", 160)#128
