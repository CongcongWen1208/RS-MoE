...
# MoE Classes for Text Prompt Encoding
class BasicExpert(nn.Module):
    def __init__(self, feature_in, feature_out):
        super().__init__()
        self.linear1 = nn.Linear(feature_in, feature_out * 4)
        self.linear2 = nn.Linear(feature_out * 4, feature_out)
...
class Blip2VicunaInstructQformerLLMLoRA(Blip2Base):
    ...
    def __init__(
        ...
        self.llm_tokenizer = LlamaTokenizer.from_pretrained(llm_model, use_fast=False, truncation_side="left",cache_dir='/scratch/sg7484/ra/data/LAVIS/output_vicuna')
        ...
        # Initialize MoE for text prompt encoding
        self.use_text_moe = use_text_moe
        if self.use_text_moe:
            moe_config = MOEConfig(
                hidden_dim=self.llm_model.config.hidden_size,
                expert_number=moe_expert_number,
                top_k=moe_top_k,
                shared_experts_number=moe_shared_experts_number
            )
            self.text_moe = ShareExpertMOE(moe_config)
            print(f"Text MoE initialized with {moe_expert_number} experts, top-k={moe_top_k}, shared experts={moe_shared_experts_number}")
    ...
    def forward(self, samples):
        ...
        # Apply MoE to text embeddings if enabled
        if self.use_text_moe:
            inputs_embeds, moe_router_logits = self.text_moe(inputs_embeds)
    ...
    def from_config(cls, cfg):
        ...
        use_text_moe = cfg.get("use_text_moe", True)
        moe_expert_number = cfg.get("moe_expert_number", 4)
        moe_top_k = cfg.get("moe_top_k", 2)
        moe_shared_experts_number = cfg.get("moe_shared_experts_number", 2)
        ...
        with lora(r, alpha, dropout, enabled=self_attention_qv_lora, qkv=[qformer_crossattention_lora_q, qformer_crossattention_lora_k, qformer_crossattention_lora_v]), custom_lora(r, alpha, dropout, enabled=(self_attention_output_lora or qformer_crossattention_lora_o), type="BertSelfOutput", sc=[self_attention_output_lora, qformer_crossattention_lora_o]), custom_lora(r, alpha, dropout, enabled=ffn_lora, type="BertOutput"):
            ...
            model = cls(
                ...
                use_text_moe=use_text_moe,
                moe_expert_number=moe_expert_number,
                moe_top_k=moe_top_k,
                moe_shared_experts_number=moe_shared_experts_number
            )
